...
const orange = "#ff5a1f";
const leadsData: { name: string; service: string; value: string; status: string; date: string }[] = [];
const campaigns: { name: string; channel: string; spend: string; leads: number; cpl: string; ctr: string; color: string }[] = [];
...
export default function Home() {
  ...
  const agentResponse = query ? `Vou analisar “${query}” com os dados cadastrados no painel e preparar um próximo passo.` : "Olá, Claudineia. Cadastre seus dados para que eu possa acompanhar ganhos, campanhas, leads e agenda com você.";
  ...
  return <div className="dashboard-shell">
    ...
    <main className="dashboard-main">
      ...
      <div className="dashboard-content">
        ...
        <section className="metrics-grid"><Metric label="Receita estimada" value="R$ 0,00" change="0%" icon={CircleDollarSign}/><Metric label="Comissões recebidas" value="R$ 0,00" change="0%" icon={DollarSign}/><Metric label="Leads gerados" value="0" change="0%" icon={UsersRound}/><Metric label="Investimento em anúncios" value="R$ 0,00" change="0%" icon={BarChart3} negative/></section>
        <section className="main-grid"><article className="panel finance-panel"><div className="panel-header"><div><p className="panel-kicker">DESEMPENHO FINANCEIRO</p><h2>Receita e comissões</h2></div><button className="more-btn"><MoreHorizontal/></button></div><div className="chart-legend"><span><i className="dot orange-dot"/> Receita estimada</span><span><i className="dot black-dot"/> Comissões</span><strong>R$ 0,00 <small>total no período</small></strong></div><div className="chart"><div className="y-axis"><span>20k</span><span>15k</span><span>10k</span><span>5k</span><span>0</span></div><div className="chart-area"><div className="grid-lines"><i/><i/><i/><i/><i/></div><svg viewBox="0 0 700 240" preserveAspectRatio="none" aria-label="Gráfico de desempenho"><path className="area orange-area" d="M0,190 C65,165 70,180 125,145 S190,132 220,150 S290,105 330,125 S390,100 430,118 S500,62 545,92 S610,42 700,24 L700,240 L0,240Z"/><path className="line orange-line" d="M0,190 C65,165 70,180 125,145 S190,132 220,150 S290,105 330,125 S390,100 430,118 S500,62 545,92 S610,42 700,24"/><path className="line black-line" d="M0,210 C70,205 75,190 125,180 S190,180 220,187 S285,155 330,168 S390,150 430,155 S500,126 545,138 S610,105 700,90"/></svg><div className="x-axis"><span>Jan</span><span>Fev</span><span>Mar</span><span>Abr</span><span>Mai</span><span>Jun</span><span>Jul</span><span>Ago</span><span>Set</span></div></div></div></article>
        <article className="panel goal-panel"><div className="panel-header"><div><p className="panel-kicker">META DO MÊS</p><h2>Rumo ao objetivo</h2></div><Target size={20} className="orange-icon"/></div><div className="goal-ring"><div><strong>74%</strong><span>alcançado</span></div></div><h3>R$ 0,00 <small>meta de receita</small></h3><div className="goal-status"><Check size={14}/> Você está Cadastre sua meta e seus ganhos para acompanhar o ritmo.</div><button className="text-button">Ver detalhes <ArrowRightIcon/></button></article></section>
        <section className="lower-grid"><article className="panel"><div className="panel-header"><div><p className="panel-kicker">OPORTUNIDADES</p><h2>Leads recentes</h2></div><button className="text-button">Ver todos <ArrowRightIcon/></button></div><div className="lead-list">{leadsData.length ? leadsData.map((lead,i)=><div className="lead-item" key={lead.name}><div className={`lead-avatar av-${i}`}>{lead.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</div><div className="lead-info"><strong>{lead.name}</strong><span>{lead.service}</span></div><div className="lead-value"><strong>{lead.value}</strong><span>{lead.date}</span></div><span className={`status status-${i}`}>{lead.status}</span><button className="more-btn"><MoreHorizontal size={17}/></button></div>) : <div className="empty-state">Nenhum lead cadastrado ainda.<br/><small>Adicione seus primeiros leads na área “Leads e clientes”.</small></div>}</div></article><article className="panel tasks-panel"><div className="panel-header"><div><p className="panel-kicker">FOCO DE HOJE</p><h2>Próximas tarefas</h2></div><button className="add-task" onClick={addTask}><Plus size={16}/></button></div><div className="task-list">{tasks.length ? tasks.map((task,i)=><label key={task} className="task-item"><input type="checkbox" onChange={e=>e.currentTarget.parentElement?.classList.toggle("done",e.currentTarget.checked)}/><span>{task}</span><small>{i===0?"10:00":i===1?"11:30":"14:00"}</small></label>) : <div className="empty-task">Nenhuma tarefa para hoje.</div>}</div><div className="new-task"><input value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTask()} placeholder="Adicionar uma tarefa..."/><button onClick={addTask}><Plus size={16}/></button></div></article></section>
        <section className="panel campaigns-panel"><div className="panel-header"><div><p className="panel-kicker">MÍDIA PAGA</p><h2>Campanhas em andamento</h2></div><button className="button-dash" onClick={() => setActiveNav("Tráfego pago")}><Plus size={15}/> Nova campanha</button></div><div className="campaign-table"><div className="campaign-head"><span>CAMPANHA</span><span>INVESTIMENTO</span><span>LEADS</span><span>CPL</span><span>CTR</span><span>STATUS</span></div>{campaigns.length ? campaigns.map(c=><div className="campaign-row" key={c.name}><div className="campaign-name"><i className={`campaign-icon ${c.color}`}><Activity size={15}/></i><span><strong>{c.name}</strong><small>{c.channel}</small></span></div><strong>{c.spend}</strong><strong>{c.leads}</strong><strong>{c.cpl}</strong><strong>{c.ctr}</strong><span className="live-status"><i/> Ativa</span></div>) : <div className="empty-state">Nenhuma campanha cadastrada ainda.<br/><small>Crie sua primeira campanha em “Tráfego pago”.</small></div>}</div></section>
      </div>
    </main>

    <button className="agent-fab" onClick={() => setAgentOpen(true)}><SparkleIcon/><span>Agente Seguralta</span></button>
    {agentOpen && <div className="agent-overlay"><div className="agent-modal"><button className="modal-x" onClick={()=>setAgentOpen(false)}><X/></button><div className="agent-heading"><div className="agent-symbol"><Lightbulb size={23}/></div><div><p className="panel-kicker">AGENTE PESSOAL</p><h2>Como posso ajudar hoje?</h2></div></div><div className="agent-message"><strong>{agentResponse}</strong>{!query && <div className="suggestions"><button onClick={()=>setQuery("Quais campanhas merecem mais investimento?")}>Analisar campanhas <ArrowRightIcon/></button><button onClick={()=>setQuery("Qual minha previsão de ganhos este mês?")}>Ver previsão financeira <ArrowRightIcon/></button><button onClick={()=>setQuery("Quais leads devo priorizar hoje?")}>Priorizar meus leads <ArrowRightIcon/></button></div>}</div><div className="agent-input"><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&setQuery(query)} placeholder="Pergunte sobre seus números..."/><button onClick={()=>setQuery(query||"Me dê um resumo do meu negócio") }><ArrowUpRight size={18}/></button></div><small>O agente usa somente os dados que você cadastrar neste painel.</small></div></div>}
  </div>;
}
